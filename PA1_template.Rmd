...
maxInterval
```
The 5-minute interval that contains the maximum of steps, averaged across all the days, is **`r maxInterval$interval`**
...
countMissingData
```
Hence, the total number of missing values in the dataset is **`r countMissingData`**.
...
```
Hence there are **`r countMissingSub`** records in the new substituted dataset
...
newMedianSteps
```
We notice that the mean of the substituted data set (**`r newMeanSteps`**) is equal to the mean of the data set without missing values (**`r meanSteps`**). The median of the substituted data set has shifted from **`r medianSteps`** to **`r newMedianSteps`**. Therefore, the mean and median for the substituted data set are almost identical to the data set without missing values.
